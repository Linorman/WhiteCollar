...
from mindspore.ops import Adam
...
class PPOTrainer:
    ...
    def step(self, queries, responses, scores):
        ...
        logprobs, ref_logprobs, values = self.batched_forward_pass(queries,
                                                                   responses)  # 拿到模型生成的tokens的log_prob、token_value
        ...
        for _ in range(self.ppo_params['ppo_epochs']):
            ...
            for i in range(bs):
                ...
                train_stats = self.train_minibatch(logprobs[idx].unsqueeze(0), values[idx].unsqueeze(0),
                                                   ...
                                                   ops.cat([queries[idx], responses[idx]]).unsqueeze(0))
        ...
        train_stats['policy/advantages'] = Tensor.flatten(train_stats['policy/advantages']).unsqueeze(0)
        train_stats['policy/advantages'] = Tensor.nan_to_num(train_stats['policy/advantages'], WANDB_PADDING)
        train_stats['policy/ratio'] = Tensor.flatten(train_stats['policy/ratio']).unsqueeze(0)
    ...
    def batched_forward_pass(self, queries, responses):
        ...
        for i in range(int(bs / fbs)):
            ...
            input_ids = self.data_collator([ops.cat([q, r]) for q, r in zip(query_batch, response_batch)])[
                "input_ids"]
    ...
    def loss(self, old_logprobs, values, rewards, query, response, model_input):
        ...
        advantages = ops.stack(advantages_reversed[::-1]).transpose(0, 1)
        ...
        logits, _, vpred = self.model(
            model_input)  # logits -> (batch, all_seq_len, vocab_size); vpred -> (batch, all_seq_len)
        logprob = logprobs_from_logits(logits[:, :-1, :], model_input[:, 1:])

        # only the generation part of the values/logprobs is needed
        logprob, vpred = logprob[:, -gen_len:], vpred[:,
                                                -gen_len - 1:-1]  # logprob -> (batch, generated_seq_len); vpred -> (batch, generated_seq_len)
        ...
        vf_loss = .5 * mean(argmax(vf_losses1, vf_losses2)[1])
        vf_clipfrac = mean(Tensor(greater(vf_losses2, vf_losses1)))
        ...
        entropy = mean(entropy_from_logits(logits))
        approxkl = .5 * mean((logprob - old_logprobs) ** 2)
        policykl = mean(logprob - old_logprobs)
        return_mean, return_var = mean(returns), Tensor.var(returns)
        value_mean, value_var = mean(values), Tensor.var(values)

        stats = dict(
            loss=dict(policy=pg_loss, value=vf_loss, total=loss),
            policy=dict(entropy=entropy, approxkl=approxkl, policykl=policykl, clipfrac=pg_clipfrac,
                        advantages=advantages, advantages_mean=mean(advantages), ratio=ratio),
            returns=dict(mean=return_mean, var=return_var),
            val=dict(vpred=mean(vpred), error=mean((vpred - returns) ** 2),
    ...
    def record_step_stats(self, kl_coef, **data):
        ...
        mean_kl = mean(ops.stack([ops.sum(kl) for kl in kl_list]))
        mean_entropy = mean(ops.stack([ops.sum(-log_probs) for log_probs in data['logprobs']]))
        mean_non_score_reward = mean(
            ops.stack([ops.sum(non_score_reward) for non_score_reward in data['non_score_reward']]))
        stats = {
